    ...
    try:
        ...
        for i in range(max_pages):
            ...
            if text1_page == text2_page:
                story.append(
                    Paragraph(
                        f"<font color='green'>✓ Identical content ({words1} words)</font>",
                        styles["Normal"],
                    )
                )
            else:
                story.append(
                    Paragraph(
                        f"<font color='red'>✗ Different content</font>",
                        styles["Normal"],
                    )
                )
                story.append(
                    Paragraph(
                        f"File 1: {words1} words | File 2: {words2} words",
                        styles["Normal"],
                    )
                )
                ...
                story.append(
                    Paragraph(f"Character difference: {diff}", styles["Normal"])
                )
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(
            json.dumps(
                {"success": False, "error": "Two PDF files required for comparison"}
            )
        )
        sys.exit(1)
