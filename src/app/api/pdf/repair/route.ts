...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await executePythonScript("repair_pdf.py", [inputPath]);

    if (!result.success) {
      return NextResponse.json(
        { success: false, error: result.error || "Failed to repair PDF" },
        { status: 500 }
      );
    }
    ...
    const outputFileName = pathParts.length > 0 ? pathParts[pathParts.length - 1] : `repaired_${Date.now()}.pdf`;
