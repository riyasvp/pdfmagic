...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await executePythonScript("extract_links_pdf.py", [inputPath]);

    if (!result.success) {
      return NextResponse.json(
        { success: false, error: result.error || "Failed to extract links" },
        { status: 500 }
      );
    }

    outputPath = result.jsonOutput as string;
    if (!outputPath) {
      return NextResponse.json(
    ...
    const outputFileName = `links_${Date.now()}.json`;
