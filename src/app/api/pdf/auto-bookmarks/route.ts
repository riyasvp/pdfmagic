...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!result.success) {
      return NextResponse.json(
        { success: false, error: result.error || "Failed to add bookmarks" },
        { status: 500 }
      );
    }
    ...
    const outputFileName = pathParts.length > 0 ? pathParts[pathParts.length - 1] : `bookmarked_${Date.now()}.pdf`;
    ...
    console.error("Auto bookmarks error:", error);
    return NextResponse.json(
