...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await executePythonScript("protect_pdf.py", [inputPath, password]);

    if (!result.success) {
      return NextResponse.json(
        { success: false, error: result.error || "Failed to protect PDF" },
        { status: 500 }
      );
    }
    ...
    const outputFileName = pathParts.length > 0 ? pathParts[pathParts.length - 1] : `protected_${Date.now()}.pdf`;
