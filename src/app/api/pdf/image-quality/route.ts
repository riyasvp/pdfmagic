...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const quality = (formData.get("quality") as string) || "70";
    ...
    const result = await executePythonScript("quality_pdf.py", [inputPath, quality]);

    if (!result.success) {
      return NextResponse.json(
        { success: false, error: result.error || "Failed to change quality" },
        { status: 500 }
      );
    }
    ...
    const outputFileName = pathParts.length > 0 ? pathParts[pathParts.length - 1] : `optimized_${Date.now()}.pdf`;
