...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await executePythonScript("embed_fonts_pdf.py", [inputPath]);

    if (!result.success) {
      return NextResponse.json(
        { success: false, error: result.error || "Failed to embed fonts" },
        { status: 500 }
      );
    }
    ...
    const outputFileName = pathParts.length > 0 ? pathParts[pathParts.length - 1] : `embedded_${Date.now()}.pdf`;
    ...
    console.error("Embed fonts error:", error);
    return NextResponse.json(
