...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await executePythonScript("grayscale_pdf.py", [inputPath]);

    if (!result.success) {
      return NextResponse.json(
        { success: false, error: result.error || "Failed to convert to grayscale" },
        { status: 500 }
      );
    }
    ...
    const outputFileName = pathParts.length > 0 ? pathParts[pathParts.length - 1] : `grayscale_${Date.now()}.pdf`;
