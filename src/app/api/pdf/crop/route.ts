...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 3. Extract crop margins option
    const cropBox = formData.get("cropBox") as string || "0,0,0,0";

    // 4. Save uploaded file
    inputPath = await saveUploadedFile(file);

    // 5. Execute Python script
    const result = await executePythonScript("crop_pdf.py", [inputPath, cropBox]);

    if (!result.success) {
      return NextResponse.json(
        { success: false, error: result.error || "Failed to crop PDF" },
        { status: 500 }
      );
    }
    ...
    const outputFileName = pathParts.length > 0 ? pathParts[pathParts.length - 1] : `cropped_${Date.now()}.pdf`;
