...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const borderWidth = (formData.get("borderWidth") as string) || "10";
    const borderColor = (formData.get("borderColor") as string) || "#000000";
    const margin = (formData.get("margin") as string) || "20";
    ...
    const result = await executePythonScript("border_pdf.py", [
      inputPath,
      borderWidth,
      borderColor,
      margin,
    ...
    if (!result.success) {
      return NextResponse.json(
        { success: false, error: result.error || "Failed to add border" },
        { status: 500 }
      );
    }
    ...
    const outputFileName = pathParts.length > 0 ? pathParts[pathParts.length - 1] : `bordered_${Date.now()}.pdf`;
