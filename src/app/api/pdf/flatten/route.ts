...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await executePythonScript("flatten_pdf.py", [inputPath]);

    if (!result.success) {
      return NextResponse.json(
        { success: false, error: result.error || "Failed to flatten PDF" },
        { status: 500 }
      );
    }
    ...
    const outputFileName = `flattened_${Date.now()}.pdf`;
