...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await executePythonScript("delete_pages_pdf.py", [inputPath, pages]);

    if (!result.success) {
      return NextResponse.json(
        { success: false, error: result.error || "Failed to delete pages" },
        { status: 500 }
      );
    }
    ...
    const outputFileName = basename(outputPath) || `deleted_${Date.now()}.pdf`;
    ...
    console.error("Delete pages PDF error:", error);
    return NextResponse.json(
